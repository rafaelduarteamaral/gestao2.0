...
export default {
  ...
  mounted() {
    if (this.isEdit) {
      this.findCfop(this.$route.params.id)
    }
  },
  ...
  methods: {
    findCfop(id) {
      this.loading = true
      this.$store.dispatch('findCfop', id).then(() => {
        this.loading = false
        this.selectCfop = this.pagCfop
        this.fillForm()
      })
    },

    fillForm() {
      this.formData = Object.assign({}, this.selectCfop)
    ...
    handleSave() {
      this.loading = true
      this.$store
        .dispatch('saveCfop', this.prepareToSave(this.formData))
        .then(() => {
          this.loading = false
          this.$message({
    ...
    handleDestroy() {
      ...
      this.$store.dispatch('destroyCfop', this.formData.id).then(() => {
        this.$message({
