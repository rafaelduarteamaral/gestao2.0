      ...
      <el-form ref="form" v-model="formData">
        <el-col :span="24">
          <el-card class="box-card">
            <div slot="header">
              <span>Informações do CstTributacao</span>
            </div>
            <el-col :span="8">
            ...
            <el-col :span="8">
              <el-form-item label="Beneficio:">
                <el-input v-model="formData.beneficio" type="text" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
...
export default {
  ...
  computed: {
    ...mapGetters({
      pagCstTributacao: 'currentCstTributacao'
    })
  },
  ...
  mounted() {
    if (this.isEdit) {
      this.findCstTributacao(this.$route.params.id)
    }
  },

  methods: {
    findCstTributacao(id) {
      this.loading = true
      this.$store.dispatch('findCstTributacao', id).then(() => {
        this.loading = false
        this.selectCstTributacao = this.pagCstTributacao
        this.fillForm()
      })
    },

    fillForm() {
      this.formData = Object.assign({}, this.selectCstTributacao)
    ...
    handleSave() {
      this.loading = true
      this.$store
        .dispatch('saveCstTributacao', this.prepareToSave(this.formData))
        .then(() => {
          this.loading = false
          this.$message({
    ...
    handleDestroy() {
      ...
      this.$store.dispatch('destroyCstTributacao', this.formData.id).then(() => {
        this.$message({
